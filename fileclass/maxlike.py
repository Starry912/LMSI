...
def maxlike(num_reads, len_msi, len_ref, num_rpt, len_hook, delta):
    ...
    while (flag == 1 and cov != 0):
        if (abs(le - len_msi) < le) or COUNT > 30:
            flag = 0
        else:
            len_msi = le 
...
def print_result(f_out,bklist,rdic,SNPlist):
    print "Wrinting_Result>>>>>>>>>>>>>>>>>>>>>>>>>>>"
    keys = list(bklist.keys())
    print >>f_out,"UNIT","\t\t","BreakPoint","\t\t","NormalParameter","\t\t","SNP"
    for key in keys:
        print >>f_out,key,"\t\t",bklist[key],"\t\t",rdic[key],"\t\t",SNPlist[key]
    return
